...
public partial class Datasheet : Form
{
    ...
    List<WiredSensor> wiredSensors = new();
    List<WirelessSensor> wirelessSensors = new();
    List<ExclusiveEntryPanel> exclusiveEntryPanels = new();
    ...
    public Datasheet(byte modbusID, Device device) : base()
    {
        ...
        if (device.exclusiveEntries.Count > 0)
        {
            foreach(ExclusiveEntry ee in  device.exclusiveEntries)
            {
                ExclusiveEntryPanel eep = new(ee.name) { Width = 495, Height = 24 };
                exclusiveEntryPanels.Add(eep);
                sensorPanel.Controls.Add(eep);
            }
        }
        ...
        try
        {
            await Task.Run(() =>
            {
                ...
                if (res)
                {
                    ...
                    for (int i = 0; i < sensorPanel.Controls.Count - exclusiveEntryPanels.Count; i++)
                    {
                ...
                if (device.exclusiveEntries.Count > 0)
                {
                    for (int i = 0; i < device.exclusiveEntries.Count; i++)
                    {
                        ExclusiveEntry e = device.exclusiveEntries[i];
                        res = PollEntry(e);
                        if (res)
                        {
                            string textValue = "";
                            ...
                            switch (e.dataFormat)
                            {
                                case DataFormat.Bool:
                                    textValue = latestMessage.Data[0] > 0x00 ? "True" : "False";
                                    break;
                                case DataFormat.UInt16:
                                    _value = (latestMessage.Data[0] << 8) + latestMessage.Data[1];
                                    textValue = _value.ToString();
                                    break;
                                case DataFormat.UInt32:
                                    _value = (latestMessage.Data[0] << 24) + (latestMessage.Data[1] << 16) + (latestMessage.Data[2] << 8) + latestMessage.Data[3];
                                    textValue = _value.ToString();
                                    break;
                                case DataFormat.String:
                                    textValue = Modbus.ByteArrayToUnicode(latestMessage.Data);
                                    break;
                                default:
                                    textValue = "n/a";
                                    break;
                            }
                            exclusiveEntryPanels[i].Invoke(new MethodInvoker(delegate { exclusiveEntryPanels[i].value.Text = textValue; }));
                        }
                    }
                }
            });
        ...
        await Task.Run(() =>
        {
            while (bytesLeft > 0)
            {
                ...
                try
                {
                    ...
                    bytesLeft -= count;
                    bytesWritten += count;
                    firmwareProgressBar.Invoke((MethodInvoker)delegate { firmwareProgressBar.Value = ((int)(bytesWritten / bytesTotal) * 100); });
...
public class ExclusiveEntryPanel : FlowLayoutPanel
{
    public Label name = new() { Width = 200, Height = 24 };
    public Label value = new() { Width = 60, Height = 24 };

    public ExclusiveEntryPanel(string name)
    {
        this.Margin = Padding.Empty;
        this.Padding = new Padding(0, 5, 0, 0);
        this.BackColor = Color.White;
        ...
        this.Controls.AddRange([this.name, this.value]);
    }
}
